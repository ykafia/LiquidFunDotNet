				...
				unsafe
				{
					fixed (ContactConstraintPoint* pointsPtr = c.Points)
					{
						...
						for (int j = 0; j < c.PointCount; ++j)
						{
							...
							float newImpulse = Common.MathB2.Clamp(ccp->TangentImpulse + lambda, -maxFriction, maxFriction);
						...
						if (c.PointCount == 1)
						{
							...
							float newImpulse = Common.MathB2.Max(ccp.NormalImpulse + lambda, 0.0f);
						...
						else
						{
							...
							for (; ; )
							{
								...
								// x' = - inv(A) * b'
								//
								Vec2 x = -Common.MathB2.Mul(c.NormalMass, b);
		...
		public bool SolvePositionConstraints(float baumgarte)
		{
			...
			for (int i = 0; i < _constraintCount; ++i)
			{
				...
				for (int j = 0; j < c.PointCount; ++j)
				{
					...
					minSeparation = Common.MathB2.Min(minSeparation, separation);

					// Prevent large corrections and allow slop.
					float C = baumgarte * Common.MathB2.Clamp(separation + Settings.LinearSlop, -Settings.MaxLinearCorrection, 0.0f);
