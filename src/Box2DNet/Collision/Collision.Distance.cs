		...
		internal unsafe void ReadCache(SimplexCache* cache, Shape shapeA, XForm transformA, Shape shapeB, XForm transformB)
		{
			...
			fixed (SimplexVertex* v1Ptr = &_v1, v2Ptr = &_v2, v3Ptr = &_v3)
			{
				...
				for (int i = 0; i < _count; ++i)
				{
					...
					v->wA = Common.MathB2.Mul(transformA, wALocal);
					v->wB = Common.MathB2.Mul(transformB, wBLocal);
					v->w = v->wB - v->wA;
					v->a = 0.0f;
				}
	...
	public partial class Collision
	{
		...
		public static unsafe void Distance(out DistanceOutput output, ref SimplexCache cache, ref DistanceInput input, Shape shapeA, Shape shapeB)
		{
			...
			while (iter < k_maxIterationCount)
			{
				...
				vertex->indexA = shapeA.GetSupport(Common.MathB2.MulT(transformA.R, p));
				vertex->wA = Common.MathB2.Mul(transformA, shapeA.GetVertex(vertex->indexA));
				//Vec2 wBLocal;
				vertex->indexB = shapeB.GetSupport(Common.MathB2.MulT(transformB.R, -p));
				vertex->wB = Common.MathB2.Mul(transformB, shapeB.GetVertex(vertex->indexB));
