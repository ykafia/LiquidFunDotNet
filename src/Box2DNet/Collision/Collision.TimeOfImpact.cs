	...
	internal struct SeparationFunction
	{
			...
			Shape shapeB, XForm transformB)
		{
			...
			if (count == 1)
			{
				...
				Vec2 pointA = Common.MathB2.Mul(transformA, localPointA);
				Vec2 pointB = Common.MathB2.Mul(transformB, localPointB);
				Axis = pointB - pointA;
				Axis.Normalize();
			}
			else if (cache->IndexB[0] == cache->IndexB[1])
			{
				...
				Vec2 normal = Common.MathB2.Mul(transformA.R, Axis);
				Vec2 pointA = Common.MathB2.Mul(transformA, LocalPoint);
				Vec2 pointB = Common.MathB2.Mul(transformB, localPointB);
			...
			else
			{
				...
				Vec2 normal = Common.MathB2.Mul(transformB.R, Axis);
				Vec2 pointB = Common.MathB2.Mul(transformB, LocalPoint);
				Vec2 pointA = Common.MathB2.Mul(transformA, localPointA);
		...
		internal float Evaluate(XForm transformA, XForm transformB)
		{
			switch (FaceType)
			{
				case Type.Points:
					{
						Vec2 axisA = Common.MathB2.MulT(transformA.R, Axis);
						Vec2 axisB = Common.MathB2.MulT(transformB.R, -Axis);
				...
				case Type.FaceA:
					{
						...
						Vec2 axisB = Common.MathB2.MulT(transformB.R, -normal);
				...
				case Type.FaceB:
					{
						...
						Vec2 axisA = Common.MathB2.MulT(transformA.R, -normal);
		...
		public static float TimeOfImpact(TOIInput input, Shape shapeA, Shape shapeB)
		{
			...
			for (; ; )
			{
				...
				if (iter == 0)
				{
					...
					target = separation > radius ? Common.MathB2.Max(radius - tolerance, 0.75f * radius) : Common.MathB2.Max(separation - tolerance, 0.02f * radius);
				}
				...
				float newAlpha = alpha;
				{
					...
					for (; ; )
					{
						...
						if (Common.MathB2.Abs(f - target) < 0.025f * tolerance)
						{
							newAlpha = x;
							break;
						}
					...
					MaxToiRootIters = Common.MathB2.Max(MaxToiRootIters, rootIterCount);
				}
			...
			MaxToiIters = Common.MathB2.Max(MaxToiIters, iter);
