...
namespace Box2DNet.Collision
{
	...
	public class CircleShape : Shape
	{
		...
		{
			Vec2 center = transform.Position + Common.MathB2.Mul(transform.R, _position);
			Vec2 d = p - center;
		...
		public override SegmentCollide TestSegment(XForm transform, out float lambda, out Vec2 normal, Segment segment, float maxLambda)
		{
			...
			Vec2 position = transform.Position + Common.MathB2.Mul(transform.R, _position);
			...
			float a = -(c + Common.MathB2.Sqrt(sigma));
		...
		public override void ComputeAABB(out AABB aabb, XForm transform)
		{
			...
			Vec2 p = transform.Position + Common.MathB2.Mul(transform.R, _position);
		...
		{
			Vec2 p = Box2DNet.Common.MathB2.Mul(xf, _position);
			...
			float area = r2 * ((float)System.Math.Asin(l / _radius) + Box2DNet.Common.Settings.Pi / 2) +
				l * Box2DNet.Common.MathB2.Sqrt(r2 - l2);
