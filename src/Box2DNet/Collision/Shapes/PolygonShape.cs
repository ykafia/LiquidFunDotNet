...
namespace Box2DNet.Collision
{
	...
	public class PolygonShape : Shape
	{
		...
		public void SetAsBox(float hx, float hy, Vec2 center, float angle)
		{
			...
			{
				_vertices[i] = Common.MathB2.Mul(xf, _vertices[i]);
				_normals[i] = Common.MathB2.Mul(xf.R, _normals[i]);
			}
		}
		...
		{
			Vec2 pLocal = Common.MathB2.MulT(xf.R, p - xf.Position);
		...
		public override SegmentCollide TestSegment(XForm xf, out float lambda, out Vec2 normal, Segment segment, float maxLambda)
		{
			...
			Vec2 p1 = Common.MathB2.MulT(xf.R, segment.P1 - xf.Position);
			Vec2 p2 = Common.MathB2.MulT(xf.R, segment.P2 - xf.Position);
			Vec2 d = p2 - p1;
			int index = -1;
			...
			{
				lambda = lower;
				normal = Common.MathB2.Mul(xf.R, _normals[index]);
		...
		{
			Vec2 lower = Common.MathB2.Mul(xf, _vertices[0]);
			Vec2 upper = lower;

			for (int i = 1; i < _vertexCount; ++i)
			{
				Vec2 v = Common.MathB2.Mul(xf, _vertices[i]);
				lower = Common.MathB2.Min(lower, v);
				upper = Common.MathB2.Max(upper, v);
			}
		...
		public override float ComputeSubmergedArea(Vec2 normal, float offset, XForm xf, out Vec2 c)
		{
			...
			Vec2 normalL = Box2DNet.Common.MathB2.MulT(xf.R, normal);
			...
			switch (diveCount)
			{
				case 0:
					if (lastSubmerged)
					{
						...
						c = Common.MathB2.Mul(xf, md.Center);
						return md.Mass;
					}
					else
					{
		...
		public override float ComputeSweepRadius(Vec2 pivot)
		{
			...
			{
				sr = Common.MathB2.Max(sr, Vec2.DistanceSquared(_vertices[i], pivot));
			}

			return Common.MathB2.Sqrt(sr);
		}
